...
// Helper function to initiate an atomic swap from Fedimint to Core Lightning.
void initiate_swap(int amount, char *lightning_node_id) {
    ...
    // Create an HTLC on the Fedimint network.
    HTLC *fedimint_htlc = new_htlc(preimage, SECRET_LENGTH, HTLC_TIMEOUT);
    ...
    printf("Fedimint HTLC: %s\n", fedimint_htlc_response);
